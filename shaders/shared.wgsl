...
var<private> AMINO_DATA: array<array<vec4<f32>, 6>, AMINO_COUNT> = array<array<vec4<f32>, 6>, AMINO_COUNT>(
    // 0  A - Alanine
    array<vec4<f32>,6>( vec4<f32>(8.5, 2.5, 0.10, 0.015), vec4<f32>(-0.2, 0.2, 0.0, 0.001), vec4<f32>(0.3, 0.3, 0.3, 1.0), vec4<f32>(0.0, 0.3, -0.73, -0.23), vec4<f32>(0.2, 0.8, 0.2, 0.7), vec4<f32>(0.3, 0.0, 0.0, 0.0) ),
    // 1  C - Cysteine
    array<vec4<f32>,6>( vec4<f32>(10.0, 2.5, 0.13, 0.02), vec4<f32>(0.0, 0.349066, 0.0, 0.001), vec4<f32>(0.1, 0.1, 0.1, 1.0), vec4<f32>(0.0, 0.3, 0.42, 0.67), vec4<f32>(0.2, 0.5, 0.5, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 2  D - Aspartic acid
    ...
    // 5  G - Glycine
    array<vec4<f32>,6>( vec4<f32>(4.0, 3.0, -0.06, 0.02), vec4<f32>(0.7, 0.1, 0.0, 0.001), vec4<f32>(0.9, 0.9, 0.9, 1.0), vec4<f32>(0.0, 0.0, 0.88, 0.88), vec4<f32>(0.3, 0.5, 0.5, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 6  H - Histidine
    array<vec4<f32>,6>( vec4<f32>(9.0, 4.0, -0.07, 0.02), vec4<f32>(0.2, -0.61, 0.0, 0.001), vec4<f32>(0.3, 0.5, 0.8, 1.0), vec4<f32>(0.0, 0.3, -0.35, -0.35), vec4<f32>(0.2, 1.2, -0.2, -0.3), vec4<f32>(1.3, 0.0, 0.0, 0.0) ),
    // 7  I - Isoleucine
    array<vec4<f32>,6>( vec4<f32>(19.0, 5.5, 0.09, 0.02), vec4<f32>(-0.3, 0.69, 0.0, 0.001), vec4<f32>(0.38, 0.38, 0.38, 1.0), vec4<f32>(0.0, 0.3, 0.61, 0.61), vec4<f32>(0.2, 0.65, 0.35, 0.7), vec4<f32>(0.3, 0.0, 0.0, 0.0) ),
    // 8  K - Lysine
    array<vec4<f32>,6>( vec4<f32>(15.0, 3.5, 0.31, 0.03), vec4<f32>(0.6, -0.16, 0.0, 0.001), vec4<f32>(0.1, 0.1, 0.2, 1.0), vec4<f32>(0.0, 0.3, -0.12, -0.12), vec4<f32>(0.2, 0.7, 0.3, 0.65), vec4<f32>(0.35, 0.0, 0.0, 0.0) ),
    // 9  L - Leucine
    array<vec4<f32>,6>( vec4<f32>(13.0, 4.5, -0.24, 0.02), vec4<f32>(-0.3332, 0.1, 0.0, 0.001), vec4<f32>(0.6, 0.5, 0.2, 1.0), vec4<f32>(0.0, 0.3, 0.95, 0.95), vec4<f32>(0.2, 0.35, 0.65, 0.4), vec4<f32>(0.6, 0.0, 0.0, 0.0) ),
    // 10 M - Methionine (START)
    array<vec4<f32>,6>( vec4<f32>(8.5, 4.0, -0.52, 0.02), vec4<f32>(0.14, -0.64, 0.0, 0.001), vec4<f32>(0.3, 0.3, 0.2, 1.0), vec4<f32>(0.0, 0.3, -0.48, -0.48), vec4<f32>(0.2, 0.8, 0.2, -0.1), vec4<f32>(1.1, 0.0, 0.0, 0.0) ),
    // 11 N - Enabler
    array<vec4<f32>,6>( vec4<f32>(16.0, 6.0, 0.21, 0.15), vec4<f32>(0.2, 0.3, 0.0, 0.001), vec4<f32>(0.1, 0.1, 0.2, 1.0), vec4<f32>(0.0, 0.0, 0.24, 0.24), vec4<f32>(0.2, 0.5, 0.5, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 12 P - Proline
    array<vec4<f32>,6>( vec4<f32>(16.0, 8.0, -0.333, 0.05), vec4<f32>(0.5, -0.1, 2.5, 0.01), vec4<f32>(0.1, 0.0, 0.2, 1.0), vec4<f32>(0.0, 0.3, -0.77, -0.77), vec4<f32>(0.2, 1.5, -0.5, 1.4), vec4<f32>(-0.4, 0.0, 0.0, 0.0) ),
    // 13 Q - Glutamine
    array<vec4<f32>,6>( vec4<f32>(8.5, 3.0, -0.221, 0.02), vec4<f32>(0.24, -0.4, 0.0, 0.001), vec4<f32>(0.34, 0.34, 0.34, 1.0), vec4<f32>(0.0, 0.3, 0.53, 0.53), vec4<f32>(0.2, 1.0, 0.0, 0.75), vec4<f32>(0.25, 0.0, 0.0, 0.0) ),
    // 14 R - Arginine
    array<vec4<f32>,6>( vec4<f32>(18.5, 3.5, -0.27, 0.04), vec4<f32>(0.5, -0.15, 0.0, 0.001), vec4<f32>(0.29, 0.29, 0.29, 1.0), vec4<f32>(0.0, 0.3, -0.29, -0.29), vec4<f32>(0.2, -0.4, 1.4, -0.3), vec4<f32>(1.3, 0.0, 0.0, 0.0) ),
    // 15 S - Serine
    array<vec4<f32>,6>( vec4<f32>(10.5, 2.5, -0.349066, 0.02), vec4<f32>(-0.349066, 0.0, 0.0, 0.001), vec4<f32>(0.0, 0.2, 0.1, 1.0), vec4<f32>(0.0, 0.2, 0.71, 0.71), vec4<f32>(0.2, 0.5, 0.5, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 16 T - Threonine
    array<vec4<f32>,6>( vec4<f32>(10.5, 3.5, 0.10112, 0.02), vec4<f32>(0.1, -0.5, 0.0, 0.001), vec4<f32>(0.45, 0.15, 0.45, 1.0), vec4<f32>(0.0, 0.2, -0.66, -0.66), vec4<f32>(0.2, 0.6, 0.4, 0.7), vec4<f32>(0.3, 0.0, 0.0, 0.0) ),
    // 17 V - Valine
    array<vec4<f32>,6>( vec4<f32>(12.0, 8.0, 0.09, 0.04), vec4<f32>(-0.3, 0.73, 0.0, 0.001), vec4<f32>(0.2, 0.3, 0.4, 1.0), vec4<f32>(0.0, 0.3, 0.36, 0.36), vec4<f32>(0.2, 0.35, 0.65, 0.6), vec4<f32>(0.4, 0.0, 0.0, 0.0) ),
    // 18 W - Tryptophan
    array<vec4<f32>,6>( vec4<f32>(16.0, 4.0, 0.349066, 0.01), vec4<f32>(0.31, -0.1, 0.0, 0.001), vec4<f32>(0.3, 0.3, 0.2, 1.0), vec4<f32>(0.0, 0.4, -0.84, -0.84), vec4<f32>(0.2, 0.55, 0.45, 0.6), vec4<f32>(0.4, 0.0, 0.0, 0.0) ),
    // 19 Y - Tyrosine
    array<vec4<f32>,6>( vec4<f32>(11.5, 4.0, -0.523599, 0.03), vec4<f32>(-0.2, 0.52, 0.0, 0.001), vec4<f32>(0.5, 0.6, 0.4, 1.0), vec4<f32>(0.0, 0.3, 0.08, 0.08), vec4<f32>(0.2, 0.25, 0.75, 0.3), vec4<f32>(0.7, 0.0, 0.0, 0.0) ),
    ...
    // 21 PROPELLER
    array<vec4<f32>,6>( vec4<f32>(16.0, 8.0, -0.523599, 0.05), vec4<f32>(0.5, -0.1, 10.0, 0.007), vec4<f32>(0.0, 0.0, 1.1, 0.0), vec4<f32>(0.0, 0.3, -0.77, -0.77), vec4<f32>(0.99, 0.45, 0.55, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 22 ALPHA SENSOR
    ...
    // 25 ALPHA_EMITTER
    array<vec4<f32>,6>( vec4<f32>(12.0, 8.0, 0.08151, 0.02), vec4<f32>(-0.13, 0.173, 0.0, 0.001), vec4<f32>(0.0, 1.0, 1.0, 0.0), vec4<f32>(0.0, 0.3, 0.36, 0.36), vec4<f32>(0.2, -0.3, 1.3, 1.2), vec4<f32>(-0.2, 0.0, 0.0, 0.0) ),
    // 26 ENABLER
    ...
    // 28 STORAGE
    array<vec4<f32>,6>( vec4<f32>(20.0, 22.0, 0.1349066, 1.3), vec4<f32>(0.31, -0.1, 0.0, 0.001), vec4<f32>(1.0, 0.5, 0.0, 10.0), vec4<f32>(0.0, 0.4, -0.84, -0.84), vec4<f32>(0.15, 0.55, 0.45, 0.6), vec4<f32>(0.4, 0.0, 0.0, 0.0) ),
    // 29 POISON RESISTANCE
    array<vec4<f32>,6>( vec4<f32>(16.0, 30.0, -0.47198, 0.02), vec4<f32>(0.1, 0.12, 0.0, 0.003), vec4<f32>(1.0, 0.4, 0.7, 0.0), vec4<f32>(0.0, 0.3, -0.58, -0.58), vec4<f32>(0.2, 0.6, 0.4, 0.55), vec4<f32>(0.45, 0.0, 0.0, 0.0) ),
    // 30 CHIRAL FLIPPER
    ...
    // 33 VAMPIRE MOUTH
    array<vec4<f32>,6>( vec4<f32>(18.5, 8.0, 0.03, 0.015), vec4<f32>(0.2, -0.33, 0.0, 0.003), vec4<f32>(1.0, 0.0, 0.0, 20.0), vec4<f32>(0.0, 0.3, 0.17, 0.17), vec4<f32>(0.2, 1.4, -0.4, 1.3), vec4<f32>(-0.3, 0.0, 0.0, 0.0) ),
    ...
    array<vec4<f32>,6>( vec4<f32>(11.0, 3.0, 0.3, 0.04), vec4<f32>(0.05, -0.1, 0.0, 0.001), vec4<f32>(0.8, 0.6, 0.2, 0.0), vec4<f32>(0.0, 0.25, 0.5, 0.5), vec4<f32>(0.15, 0.6, 0.4, 0.7), vec4<f32>(0.4, 0.0, 0.0, 0.0) ),
    ...
    array<vec4<f32>,6>( vec4<f32>(10.5, 2.8, 0.4, 0.05), vec4<f32>(0.0, 0.12, 0.0, 0.00005), vec4<f32>(1.0, 0.3, 0.4, 0.0), vec4<f32>(0.0, 0.3, 0.4, 0.4), vec4<f32>(0.1, 0.5, 0.5, 0.5), vec4<f32>(0.5, 0.0, 0.0, 0.0) ),
    // 42 ANCHOR
